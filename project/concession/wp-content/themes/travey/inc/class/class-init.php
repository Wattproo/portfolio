...
class Init {
	...
	public function change_stylesheet_directory() {
		return TRAVEY_DIR . 'gutenverse-files';
	}
	...
	public function template_path( $template_file, $theme_slug, $template_slug ) {
		switch ( $template_slug ) {
			...
			case 'destination':
				return $this->change_stylesheet_directory() . '/templates/destination.html';
			case 'faq':
			...
			case 'packages-detail':
				return $this->change_stylesheet_directory() . '/templates/packages-detail.html';
			case 'packages':
				return $this->change_stylesheet_directory() . '/templates/packages.html';
			case 'page':
	...
	public function notice_closed() {
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'travey_admin_notice' ) ) {
	...
	public function notice_install_plugin() {
		...
		if ( isset( $screen->parent_file ) && 'themes.php' === $screen->parent_file && 'appearance_page_travey-dashboard' === $screen->id ) {
			return;
		}
		...
		$button_text = __( 'Install Required Plugins', 'travey' );
		...
		<style>
			.install-gutenverse-plugin-notice {
				...
				background: url( <?php echo esc_url( TRAVEY_URI . '/assets/img/background-banner.png' ); ?> );
		...
		jQuery( function( $ ) {
			$( 'div.notice.install-gutenverse-plugin-notice' ).on( 'click', 'button.notice-dismiss', function( event ) {
				...
				$.post( ajaxurl, {
					...
					nonce: '<?php echo esc_html( wp_create_nonce( 'travey_admin_notice' ) ); ?>',
				} );
			} );
			...
			<div class="gutenverse-notice-inner">
				<div class="gutenverse-notice-content">
					<div class="gutenverse-notice-text">
						<h3><?php esc_html_e( 'Take Your Website To New Height with', 'travey' ); ?> <span>Gutenverse!</span></h3> 
						<p><?php esc_html_e( 'Travey theme work best with Gutenverse plugin. By installing Gutenverse plugin you may access Travey templates built with Gutenverse and get access to more than 40 free blocks, hundred free Layout and Section.', 'travey' ); ?></p>
					...
					<div class="gutenverse-notice-image">
						<img src="<?php echo esc_url( TRAVEY_URI . '/assets/img/banner-install-gutenverse-2.png' ); ?>"/>
					</div>
				</div>
			</div>
		</div>
	...
	public function backend_scripts() {
		...
		if ( $screen->id === 'appearance_page_travey-dashboard' ) {
			...
			wp_localize_script( 'travey-dashboard', 'GutenverseThemeConfig', $this->theme_config() );
		}
	}
	...
	public function theme_config() {
		return array(
			'demo'    => esc_url( 'https:/gutenverse.com/demo?name=travey' ),
			'pages'   => array(
